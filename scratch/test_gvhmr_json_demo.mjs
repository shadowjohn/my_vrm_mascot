...
assert.match(html, /main\s*\{[^}]*height:\s*calc\(100vh - 65px\)/s);
assert.match(html, /aside\s*\{[^}]*overflow-y:\s*auto/s);
assert.match(html, /#sourcePath\s*\{[^}]*word-break:\s*break-all/s);
assert.match(html, /id="frameDebug"/);
assert.match(html, /function nearestKeyAt\(keys, timeMs\)/);
assert.match(html, /leftHandPose:\s+nearestKeyAt\(state\.motion\?\.hand_poses\?\.left, timeMs\)/);
assert.match(html, /rightHandPose:\s+nearestKeyAt\(state\.motion\?\.hand_poses\?\.right, timeMs\)/);
assert.match(html, /fetch\('api\/gvhmr\/demo-motions',\s*\{\s*cache:\s*'no-store'\s*\}\)/);
assert.match(html, /fetch\(state\.skeletonUrl,\s*\{\s*cache:\s*'no-store'\s*\}\)/);
assert.match(html, /fetch\(url,\s*\{\s*cache:\s*'no-store'\s*\}\)/);
...
for (const path of demoJsonPaths) {
  ...
  assert.ok(demoPayload.hand_poses?.left?.length > 0, `${path} must include left hand pose layer`);
  assert.ok(demoPayload.hand_poses?.right?.length > 0, `${path} must include right hand pose layer`);
  assert.ok(demoPayload.bones?.leftHand?.length > 0, `${path} must include leftHand wrist keys`);
  assert.ok(demoPayload.bones?.rightHand?.length > 0, `${path} must include rightHand wrist keys`);
